...
namespace Slideshow
{
    ...
    {
        public async static Task<IReadOnlyList<StorageFile>> GetImageList(StorageFolder folder)
        {
        ...
        public static async Task<XmlDocument> CreateUpdate(StorageFile file)
        {
            try
            {
                if (file == null)
        ...
        public static async void Run(int seconds, IReadOnlyList<StorageFile> fileList)
        {
            ...
            var tile = await GetImages.CreateUpdate(file);
            if (tile != null)
            {
